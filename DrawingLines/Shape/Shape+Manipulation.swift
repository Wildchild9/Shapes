...
public extension Shape {
    ...
    public func joinedAt(x: CGFloat, y: CGFloat) -> Shape {
        var newShape = self
        newShape.add(.break)
    ...
    public func joinedAt(points: (x: CGFloat, y: CGFloat)...) -> Shape {
    ...
    public func joinedAt(points: [CGPoint]) -> Shape {
        guard !points.isEmpty else { return self }
        var arr = self.break()
        for case let .point(px, py) in components {
            for point in points {
    ...
    public func joinedAt(points: [Component]) -> Shape {
        guard !points.isEmpty else { return self }
        var arr = self.break()
        for case let .point(px, py) in components {
    ...
    public func mapPoints(_ transform: (CGPoint) throws -> CGPoint) rethrows -> [Component] {
