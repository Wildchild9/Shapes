...
let cube = Shape().to(0, 0).to(0, 100).to(100, 100).to(100, 0).close().prism().scaled(by: 1.25).applyingOptions(.fill(.argon), .showPoints)
...
let triangularPrism = Shape().to(0, 100).to(100,100).to(50, 0).close().scaled(by: 1.25).prism(offsetByX: 70, y: 20).applyingOptions(.fill(.alihossein), .lineWidth(3), .showPoints)
...
let squareBasedPyramid = Shape().to(0, 100).to(100, 100).to(125, 75).to(25, 75).close().joinedAt(x: 60, y: -30).applyingOptions(.fill(.netflix), .showPoints)





let randomShape = Shape.random(in: 1...250, numberOfPoints: 20).scaled(by: 1.25).withFill(.rainbowBlue)

let s = Shape(components: .point(0, 0)).to(100, 0).to(100, 100).to(0, 100).close().scaled(by: 1.4).showingPoints()
let s2 = s.prism().applyingOptions(.fill(.backToEarth), .showPoints, .pointFill(.relativeToBackground)) + s.translatedBy(x: 12.5, y: -12.5)


let squareBipyramid = Shape().to(0, 100).to(100, 100).to(125, 75).to(25, 75).close().joinedAt(points: (60, -30), (65, 205)).withFill(.wireTap, angle: 225).showingPoints()
squareBipyramid.rotated(by: 45).showingPoints(withFill: .relativeToBackground)

var tetrahedron = Shape().to(0, 100).to(100, 100).to(50, 65).close().joinedAt(x: 50, y: -30).scaled(by: 2).showingPoints()


var joiningSquares = Shape().to(0, 0).to(0, 100).to(100, 100).to(100, 0).close().to(50, 50).to(150, 50).to(150, 150).to(50, 150).close().withFill(.instagram)

joiningSquares.options.fillInShape = true
joiningSquares

var diamond = Shape().to(0, 0).to(100, 0).to(100, 100).to(0, 100).close()
let d = diamond + diamond.transformed { CGPoint(x: $0.x + 10, y: $0.y + 10) }
d.showingPoints()
...
let pyramid = Shape().to(0, 100).to(100, 100).to(125, 75).to(25, 75).close().joinedAt(x: 60, y: -50).withFill(.martini)
...
var trigonalPlanar = Shape().to(50, 0).break().to(0, 100).break().to(100, 100).break().joinedAt(x: 50, y: 50)
