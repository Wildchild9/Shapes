...
let cube = Shape().to(0, 0).to(0, 100).to(100, 100).to(100, 0).close().prism().scaled(by: 1.25).withFill(.argon)
...
let triangularPrism = Shape().to(0, 100).to(100,100).to(50, 0).close().scaled(by: 2).withFill(.alihossein).prism(offsetByX: 70, y: 20)
...
let squareBasedPyramid = Shape().to(0, 100).to(100, 100).to(125, 75).to(25, 75).close().joinedAt(x: 60, y: -30).withFill(.martini)
...
let randomShape = Shape.random(in: 1...250, numberOfPoints: 15).scaled(by: 1.25).withFill(.rainbowBlue)

let s = Shape(components: .point(0, 0)).to(100, 0).to(100, 100).to(0, 100).close().scaled(by: 1.4)
s.prism().withFill(.backToEarth) + s.translatedBy(x: 12.5, y: -12.5)


let tetrahedron = Shape().to(0, 100).to(100, 100).to(125, 75).to(25, 75).close().joinedAt(points: (60, -30), (60, 205)).withFill(.ibizaSunset, angle: 225).rotated(by: 90)
