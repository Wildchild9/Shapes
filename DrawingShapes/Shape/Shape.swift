...
public extension Shape {
    ...
    init (components: [Component], fill: Gradient) {
        self.init(components: components)
        self.fill = fill
    }
...
public extension Shape {
    static func random(in range: ClosedRange<CGFloat>, numberOfPoints: Int) -> Shape {
...
public extension Shape {
    ...
    /// The number of points that make up the shape.
    ///
    /// - Complexity: O(*n*) where *n* is the number of points in the shape.
    ///
...
public extension Shape {
    ...
    mutating func append(_ components: [Component]) {
        for component in components {
            append(component)
        }
    }
    ...
    mutating func append(_ newComponents: [[Component]]) {
    ...
    func appending(_ components: [Component]) -> Shape {
        var newShape = self
...
public extension Shape {
    ...
    func withFill(_ gradient: Gradient, angle: CGFloat = 0.0) -> Shape {
        var newShape = self
        newShape.fill = gradient.withAngle(of: angle)
        return newShape
    }
    
    func showingPoints(withFill pointFill: Options.PointFill? = nil) -> Shape {
        var newShape = self
    ...
    func hidingPoints() -> Shape {
        var newShape = self
        newShape.options.displayPoints = false
        return newShape
    }
    
    mutating func applyOption(_ option: AllOptions) {
        switch option {
    ...
    mutating func applyOptions(_ options: AllOptions...) {
        options.forEach { applyOption($0) }
    }
    
    func applyingOption(_ option: AllOptions) -> Shape {
        var newShape = self
        newShape.applyOption(option)
        return newShape
    }
    func applyingOptions(_ options: AllOptions...) -> Shape {
        var newShape = self
